...
import { Users, Search, ShieldAlert, Lock, CheckCircle2, AlertCircle, Sparkles, Check, X } from 'lucide-react';
...
export default function AdminUsersPage() {
  ...
  const handleToggleVerification = async (userId: string, currentVerified: boolean) => {
    const reason = prompt(`Enter reason for ${currentVerified ? 'removing' : 'granting'} Verified Creator Checkmark:`);
    if (!reason) return;
    ...
    try {
      const res = await fetch('/api/v1/admin/users', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'toggle_verification', userId, isVerified: !currentVerified, reason }),
      });
      const data = await res.json();
      if (data.success) {
        setMsg(`Creator verification badge ${!currentVerified ? 'granted' : 'removed'}`);
        fetchUsers(q);
        setTimeout(() => setMsg(''), 3000);
      }
    } catch (e) {}
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-800">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-white flex items-center gap-2">
            ...
            <span>User Management & Verification Desk</span>
          </h1>
          <p className="text-xs text-slate-400">Manage user status, wallet freeze states, and grant Verified Creator Checkmarks</p>
        </div>
      ...
      {msg && (
        <div className="p-3 rounded-xl bg-indigo-950/60 border border-indigo-800 text-indigo-300 text-xs font-semibold flex items-center gap-2">
            ...
            <table className="w-full text-left text-xs min-w-[600px]">
              <thead>
                <tr className="border-b border-slate-800 text-slate-400">
                  ...
                  <th className="pb-3 font-semibold">Verified Badge</th>
              ...
              <tbody className="divide-y divide-slate-800/60">
                {users.map((u) => {
                  const isVerified = u.accountType === 'CREATOR' || !!u.profile?.isVerified;
                  return (
                    <tr key={u.id} className="hover:bg-slate-850">
                      <td className="py-3">
                        <span className="font-bold text-white block flex items-center gap-1">
                          ...
                          {isVerified && <Sparkles className="w-3.5 h-3.5 text-amber-400 fill-amber-400 shrink-0" />}
                        </span>
                      ...
                      <td className="py-3">
                        ...
                        >
                          ...
                          <span>{isVerified ? 'Verified Creator' : 'Unverified'}</span>
                        </button>
                      </td>
                      ...
                      <td className="py-3 text-right space-x-1.5">
                        ...
                        >
                          {u.wallet?.isFrozen ? 'Unfreeze' : 'Freeze'}
                        </button>
                      </td>
                    </tr>
                  );
